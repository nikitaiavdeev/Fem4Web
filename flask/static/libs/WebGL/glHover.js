glHover = function () {
	...
	this.pixels; // read pixeles from the screen
	
	this.item; // hovered node or element	
	this.id;
	this.type;
...
glHover.prototype = {
	...
	mouseHover:function(){
		...
		if ( hoverIndex > 0 ){
			...
			if( id != this.id || type != this.type ){
				...
				this.coords.length = type == 4 ? (type+2)*3 : type*3;
				...
				if( type == 1 ){
					...
					this.coords.insertArr(glNodes.coords, 0, 
							...
							off[0], off[2], off[3]]  // 2nd triangle
						);
	...
	leftClick:function(){
		...
		if ( !model.isLoaded || !this.item || !selection.textBox ){	return; }
