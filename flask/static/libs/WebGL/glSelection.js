...
glSelection.prototype = {
	...
	elmIntersect:function(elm, rX1, rX2, rY1, rY2){
		let off, x = [], y = [],
			rX = [rX1, rX2, rX2, rX1, rX1], rY = [rY1, rY1, rY2, rY2, rY1];
	...
	pointInside:function(x1, y1, x, y){
		let s, s0;
	...
	sign:function(x1, y1, x2, y2, x3, y3){
		return (x1 - x3) * (y2 - y3) - (x2 - x3) * (y1 - y3);
	},
	setSelection:function(){
		for(node of this.selectList.nodeArr){
			node.select = true;
			node.setStage();
		}
		for(elm of this.selectList.elmArr){
			elm.select = true;
			elm.setStage();
		}
		model.updateStages();
	},
	clearSelection:function(update = true){
		for(node of fmNodes){
			node.select = false;
			node.setStage();
		}
