	...
	this.div.addEventListener("click", function(e) {
		...
		if (KeyboardState.Shift) {
			if( tree.lastSelectedRow > -1 ){
				for( let i = Math.min(this.rowIndex, tree.lastSelectedRow); 
						i <= Math.max(this.rowIndex, tree.lastSelectedRow); i++){
			...
			for (let i = Math.min(tree.firstSelectedRow, tree.lastSelectedRow); 
					i <= Math.max(tree.firstSelectedRow, tree.lastSelectedRow) ; i++) {
				if( !trs[i].classList.contains('selected') ){
...
treeControl.prototype = {
	addGroup:function(newGroup) {	
		...
		if( this.container.offsetHeight > tree.MAX_HEIGHT ){this.module.style.height = this.MAX_HEIGHT+'px'}
	},
	...
	toggleGroupVisibility:function(td){
	...
	toggleSelectedGroupsVisibility:function(){
		let trs = this.table.getElementsByClassName('treeTableRow'),
			td, v, visibility;
