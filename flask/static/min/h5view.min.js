var canvas=null;var textCanvas=null;var gl=null;var ctx=null;var csel=null;var camera=null;var render=null;var model=null;var cidAxis=null;var bdfData=null;var hover=null;var selection=null;var measure=null;var loader=null;var animationID=null;window.onload=function(){let imgUrls=['/static/img/build_status.svg','/static/img/Logo.svg','/static/img/ribbon/backView.svg','/static/img/ribbon/bottomView.svg','/static/img/ribbon/compass.svg','/static/img/ribbon/crossSection.svg','/static/img/ribbon/deform.svg','/static/img/ribbon/fitView.svg','/static/img/ribbon/fringe.svg','/static/img/ribbon/frontView.svg','/static/img/ribbon/help.svg','/static/img/ribbon/import.svg','/static/img/ribbon/import_bdf.svg','/static/img/ribbon/import_csv.svg','/static/img/ribbon/import_ses.svg','/static/img/ribbon/isoView.svg','/static/img/ribbon/labelIds.svg','/static/img/ribbon/leftView.svg','/static/img/ribbon/measure.svg','/static/img/ribbon/openFile.svg','/static/img/ribbon/toggleBackground.svg','/static/img/ribbon/properties.svg','/static/img/ribbon/rightView.svg','/static/img/ribbon/settings.svg','/static/img/ribbon/shadedRander.svg','/static/img/ribbon/showHide.svg','/static/img/ribbon/shrinkElements.svg','/static/img/ribbon/toggleEdges.svg','/static/img/ribbon/toggleNodes.svg','/static/img/ribbon/topView.svg','/static/img/ribbon/transparentRander.svg','/static/img/ribbon/wireframeRander.svg','/static/img/gui/checked.svg','/static/img/gui/close.svg','/static/img/gui/eye.svg','/static/img/gui/hide.svg','/static/img/gui/search.svg','/static/img/gui/toggleTree.svg'],imgCount=0,len=imgUrls.length;for(const url of imgUrls){let img=new Image();img.src=url;img.onload=()=>{if(++imgCount==len){initH5View();}};}};function initH5View(){canvas=document.getElementById('glCanvas3D');textCanvas=document.getElementById('textCanvas');selectCanvas=document.getElementById('selectCanvas');try{gl=canvas.getContext('webgl2');ctx=textCanvas.getContext('2d');csel=selectCanvas.getContext('2d');}catch(e){alert('Could not initialise WebGL, sorry :-(\n'+e);return;}
if(!gl){alert('Could not initialise WebGL, sorry :-(');return;}
camera=new $glCamera();render=new $glRender();cidAxis=new $cidAxis();hover=new $glHover();selection=new $glSelection();measure=new $glMeasurements();initListeners();reportWindowSize();render.drawScene();loader=document.getElementById('initialLogo').style;loader.backgroundColor='transparent';loader.opacity=1;loaderFade();}
function loaderFade(){if((loader.opacity-=0.25)<0)
...
addBtn(text,icon,isSmall,is2ndRow=false){let btn=document.createElement('button');btn.classList.add('ribbonButton');btn.innerHTML=text;btn.style.backgroundImage="url(static/img/ribbon/"+icon+".svg)";if(isSmall==false)
btn.setAttribute('size','large');this.addItem(btn,isSmall,is2ndRow);return btn;}
addDropDown(text,icon,isSmall,isSelectable,is2ndRow=false){let wrap=document.createElement('div'),btn=document.createElement('button'),dropDown=document.createElement('div');btn.classList.add('ribbonDropdownBtn');btn.innerHTML=text;btn.style.backgroundImage="url(static/img/ribbon/"+icon+".svg)";if(!isSmall)
...
addDropDownItem(dropDown,text,icon){let btn=document.createElement('button');btn.classList.add('ribbonButton');btn.style.backgroundImage="url(static/img/ribbon/"+icon+".svg)";btn.innerHTML=text;dropDown.nextSibling.appendChild(btn);btn.addEventListener('click',this.onDropDownOptionClick,false);}
...
onDropDownOptionClick(e){if(this.parentElement.previousSibling.getAttribute('changeicon')!=='no'){this.parentElement.previousSibling.innerHTML=this.innerHTML;this.parentElement.previousSibling.style.backgroundImage=this.style.backgroundImage;}
...
expand(newNode){}
collapse(newNode){}}
class treeControl{constructor(){this.MAX_HEIGHT=650;this.module=document.getElementById('treeView');this.showBtn=document.createElement('button');this.hideBtn=document.createElement('button');this.showBtn.id='showTree';this.showBtn.style.display='none';this.hideBtn.classList.add('hideBtn');this.hideBtn.onclick=this.showBtn.onclick=function(){tree.container.style.display=tree.container.style.display==='none'?'':'none';tree.showBtn.style.display=tree.showBtn.style.display==='none'?'':'none';tree.hideBtn.style.display=tree.hideBtn.style.display==='none'?'':'none';tree.module.classList.toggle('hidden');tree.module.classList.toggle('resizable');};this.module.appendChild(this.showBtn);this.module.appendChild(this.hideBtn);this.container=document.createElement('div');this.container.id='treeContent';this.module.appendChild(this.container);this.filter=document.createElement('input');this.filter.type='text';this.filter.id='treeFilter';this.filter.placeholder='Groups filter..';this.container.appendChild(this.filter);this.head=document.createElement('div');this.head.classList.add('treeTableHead');this.container.appendChild(this.head);let div=document.createElement('div');div.classList.add('treeTableCell','col1');div.innerHTML='Name';this.head.appendChild(div);div=document.createElement('div');div.classList.add('treeTableCell','col2');div.id='treeH2';this.head.appendChild(div);div.onclick=function(e){e.stopPropagation();tree.toggleAllGroupsVisibility();};this.table=document.createElement('div');this.table.classList.add('treeTable');this.container.appendChild(this.table);this.count=0;this.maxHeight=null;this.firstSelectedRow=null;this.lastSelectedRow=null;this.documentClickAdded=false;this.module.addEventListener("mouseenter",function(e){this.classList.add("hovered");},false);this.module.addEventListener("mouseleave",function(e){this.classList.remove("hovered");},false);this.filter.onkeyup=function(){let trs=tree.table.getElementsByClassName('treeTableRow'),filter=tree.filter.value.toUpperCase(),txtValue,td,count;for(let i=0;i<trs.length;i++){td=trs[i].firstChild;if(td){txtValue=td.innerText;if(txtValue.toUpperCase().indexOf(filter)>-1){trs[i].style.display="flex";count++;}else{trs[i].style.display="none";}}}
...
model.updateStages();model.updateSelectStages();}};const tree=new treeControl();document.onkeydown=function(e){if(e.keyCode==32){tree.toggleSelectedGroupsVisibility();}
if(tree.documentClickAdded&&e.keyCode==27){tree.documentClickAdded=false;tree.removeAllSelect();}}
class $sideBar{constructor(){this.wrapper=document.getElementById('sideBar');this.closeBtn=document.createElement('button');this.closeBtn.classList.add('closeBtn');this.wrapper.appendChild(this.closeBtn);this.closeBtn.onclick=this.closeBtnClick;this.mouseInitX=null;this.dragWidth=document.createElement('div');this.dragWidth.classList.add('dragWidthLeft');this.dragWidth.addEventListener('mousedown',this.dragWidthClick);this.wrapper.appendChild(this.dragWidth);this.table=document.createElement('div');this.table.classList.add('sideBarContent');this.table.id='sideBarContent';this.wrapper.appendChild(this.table);this.btnContainer=document.createElement('div');this.btnContainer.id='sideBarBtnContent';this.wrapper.appendChild(this.btnContainer);this.inputCount=0;this.currentFunction=null;this.currentObject=null;this.focusElement=null;this.timeout=null;}
...
if(typeof window[inpObject]!=='object'){let script=document.createElement('script');script.src=source;document.getElementsByTagName('head')[0].appendChild(script);}else{this.currentObject=new window[inpObject]();}
...
addCloseBtn(wrap){const button=document.createElement('button');button.classList.add('closeBtn');button.addEventListener('click',this.onCloseClick);wrap.appendChild(button);}
...
return arr;}}
const loadModel=new $loadModel();class $importBdf{constructor(inpFiles){let reader=new FileReader();loaderShow();model=new $glMesh();reader.onloadend=this.readBdf;this.filesCount=inpFiles.length;this.filesReaded=0;this.clearGlobals();for(const inpFile of inpFiles){reader.readAsBinaryString(inpFile);}}
...
clearGlobals(){glNodes.count=0;glNodes.coords=[];glNodes.colors=[];glNodes.selColors=[];glNodes.stage=[];glBars.count=0;glTrias.count=0;glQuads.count=0;}
...
parseBdfStr(str){const f=parseFloat(str);if(isNaN(f))
...
return line_list;}}
let importBdf=null;class $importGroups{constructor(inpFiles){let inpFile=inpFiles[0];let reader=new FileReader();loaderShow();reader.onloadend=function(e){const fr=e.target;if(fr.readyState==FileReader.DONE){let groupName,lineData,curGroup,fileData=fr.result.replace(/" [\/][\/] @(\r\n|\n)"/g,'');for(const[key,node]of Object.entries(fmElemsDict)){elm.groupShow=0;elm.groupSelect=0;}
